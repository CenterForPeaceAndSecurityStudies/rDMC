...
```{r include = FALSE}
country_meta <- read.csv(file = paste0(here::here(), '/data/aux/IISS_supp_iiss_countries.csv'))
```
...
The file 01b_prepIISS_supp.Rmd deals with typos and formatting errors associated with military equipment (equipment_type, equipment_subtype, equipment_name, equipment_subname, unit_name). The file is read in and run two times to ensure that all coding errors are fixed. 
...
for(i in c(1:2)){
  source_rmd(paste0(here::here(), "/docs/02aux_prep_clean-supp.Rmd"))
}
...
*2022-04-26 Fix the following equipment types:*
miscellaneous = UK 1983-1985 repair forwardrepair
...
write.csv(abbs, paste0(here::here(), "/data/aux/","equipment_name_abbrevs.csv"))
```
...
df
saveRDS(df,paste0(here::here(),"/data/aux/","Country_Year.rds"))
```
...
saveRDS(iiss_raw, paste0(here::here(), "/data/", "rDMC_raw_v1.rds"))
write.csv(iiss_raw, paste0(here::here(), "/data/", "rDMC_raw_v1.csv"))
```
