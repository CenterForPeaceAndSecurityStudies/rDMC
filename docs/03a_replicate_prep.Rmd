...
## Load and subset
```{r}
lw <- haven::read_dta(file = paste0(here::here(),'/inst/extdata/Lyall-Wilson_IO_2009/Rage_RepData.dta')) %>%
...
rdmc_raw <- readRDS(file = paste0(here::here(), "/data/01z_rDMC_raw.rds")) %>%
  dplyr::mutate(ccode = as.numeric(ccode))

rdmc_lw <- rdmc_raw %>%
  dplyr::select(year, ccode, equipment_type, equipment_subtype, equipment_name, unit_count) %>%
  tidyr::unite(tek, c(equipment_type, equipment_subtype, equipment_name), sep = "_", remove = TRUE) %>%
  dplyr::filter(tek %in% c("armoured fighting vehicles_MICV_AIFV",
                           "armoured fighting vehicles_NA_ACV",
                           "armoured fighting vehicles_NA_AFV",
                           "armoured fighting vehicles_NA_AIFV",
                           "armoured fighting vehicles_NA_AIFV/APC",
                           "armoured fighting vehicles_NA_ASLT",
                           "armoured fighting vehicles_NA_ATK",
                           "armoured fighting vehicles_NA_HY TK",
                           "armoured fighting vehicles_NA_IFV",
                           "armoured fighting vehicles_NA_LAV",
                           "armoured fighting vehicles_NA_LFAV",
                           "armoured fighting vehicles_NA_MBT",
                           "armoured fighting vehicles_NA_MICV",
                           "armoured fighting vehicles_NA_RECCE/AIFV/APC",
                           "armoured fighting vehicles_NA_TK",
                           "armoured fighting vehicles_NA_VCTP",
                           "armoured fighting vehicles_AFV_RECCE",
                           "armoured fighting vehicles_APC_APC(T)",
                           "armoured fighting vehicles_NA_APC(T)",
                           "armoured fighting vehicles_NA_APC(T)/(W)",
                           "armoured fighting vehicles_NA_ARMD",
                           "armoured fighting vehicles_NA_APC",
                           "armoured fighting vehicles_NA_RECCE",
                           "armoured fighting vehicles_NA_SCOUT",
                           "armoured fighting vehicles_scout cars_RECCE",
                           "anti-tank/anti-infrastructure_NA_SP",
                           "anti-tank/anti-infrastructure_RCL_SP",
                           "anti-tank/anti-infrastructure_RL_SP",
                           "anti-tank/anti-infrastructure_SP_NA",
                           "armoured fighting vehicles_NA_LT TK",
                           "armoured fighting vehicles_NA_MED TK")) %>%
  dplyr::group_by(year, ccode) %>%
  dplyr::summarise(mechvec_rdmc = sum(unit_count, na.rm = TRUE)) %>%
  dplyr::ungroup()
```
...
rdmc_lw <- dplyr::left_join(rdmc_lw, lw) %>%
  dplyr::distinct()

# Get list of country names to make filtering easier
rdmc_list <- rdmc_raw %>%
  dplyr::select(year, country, ccode) %>%
  dplyr::distinct()

rdmc_lw <- dplyr::left_join(rdmc_lw, rdmc_list) %>%
  dplyr::select(year, country, ccode, dplyr::starts_with("mechvec")) %>%
  dplyr::mutate(mechvec_diff = abs(mechvec_rdmc - mechvec_lw))

# See correlation between the two
rdmc_lw %>%
  dplyr::select(mechvec_rdmc, mechvec_lw) %>%
  GGally::ggpairs()

# Identify differences in values
rdmc_lw %>%
  ggplot(aes(x = mechvec_diff)) +
  geom_histogram()

rdmc_lw %>%
  dplyr::arrange(dplyr::desc(mechvec_diff)) %>%
  DT::datatable()
...
### New data
See if using new data shows anything different
